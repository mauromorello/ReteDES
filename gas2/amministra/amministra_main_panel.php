...
//--------------------------------------------CONTENUTO
      ...
      $n_dsn = db_prezzi_dettaglio_diversi_prezzi_articoli();
      if($n_dsn==0){
        $dettagli_prezzi='<div style="padding:6px;margin-bottom:2px;" class="ui-corner-all campo_ok">DETTAGLI ORDINE -> PREZZI ARTICOLI : <b>OK</b></div>';
      }else{
        $dettagli_prezzi='<div style="padding:6px;margin-bottom:2px;" class="ui-corner-all campo_mio">Ci sono <b>'.$n_dsn.'</b> dettagli con prezzi diversi da articoli</div>';
      }
      ...
      //PERSEFONE
      $sql = "SELECT YEAR(last_activity) as ANNO ,MONTH(last_activity) as MESE ,count(*) As QUANTI from maaking_users GROUP BY YEAR(last_activity), MONTH(last_activity)";
        $res = $db->sql_query($sql);
        $ultima_visita ='<div style="padding:6px;margin-bottom:2px;" class="ui-corner-all campo_ok">';
        while($row = $db->sql_fetchrow($res)) {
            $toti += $row["QUANTI"];
            $perc = round(($toti / $quanti) * 100,1);
            $ultima_visita .= "".$row["ANNO"]."-".$row["MESE"].": <strong>".$row["QUANTI"]."</strong> tot : $toti ---> ($perc%)<br>";
        }
        $ultima_visita .="</div>";
      //ISCRIZIONE

        unset($toti);
        $sql = "SELECT YEAR(regdate) as ANNO ,MONTH(regdate) as MESE ,count(*) As QUANTI from maaking_users GROUP BY YEAR(regdate), MONTH(regdate)";
        $res = $db->sql_query($sql);
        $iscrizione ='<div style="padding:6px;margin-bottom:2px;" class="ui-corner-all campo_ok">';
        while($row = $db->sql_fetchrow($res)) {
            $toti += $row["QUANTI"];
            $perc = round(($toti / $quanti) * 100,1);
            $iscrizione .= "".$row["ANNO"]."-".$row["MESE"].": <strong>+".$row["QUANTI"]."</strong> tot : $toti ---> ($perc%)<br>";
        }
        $iscrizione.="</div>";
      // COSTRUZIONE TABELLA  -----------------------------------------------------------------------
