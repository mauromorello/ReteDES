<?php
    ...
    function ordine_render_do_edit_descrizione($id_ordine){
        ...
        if($e_total==0){
             ...
             $descrizione_q =sanitize($descrizione);
             $note_ordine_q = sanitize($note_ordine);

             (int)$id_ordine;


             $query = "UPDATE retegas_ordini SET
                                descrizione_ordini='$descrizione_q',
                                note_ordini='$note_ordine_q'
                                WHERE id_ordini='$id_ordine'
                                LIMIT 1;";
    ...
    function ordine_render_edit_partecipazione($id_ordine,$id_gas){
        ...
        while ($row = mysql_fetch_array($result)){
            ...
            $gas_perm = read_option_gas_text_new($id_gas_table,"_GAS_PUO_PART_ORD_EST");
    ...
    function ordine_render_edit_switch_listino($id_ordine){
        ...
        while ($rows = mysql_fetch_array($result)){
                ...
                if(listini_compatibili($idtip,$id_listino)){
 ...
 $h = '<div class="rg_widget rg_widget_helper">
    ...
    function ordine_render_do_edit_switch_listino($id_ordine){
       ...
       $log .="Listino VECCHIO =".$old_listino." Listino NUOVO = $id_listino, ID Ordine = $id_ordine<br>";
        ...
        if ($crc_new==$crc_old){
            ...
            $query_new_list=("UPDATE retegas_dettaglio_ordini
                                SET retegas_dettaglio_ordini.id_articoli = '$new_articolo',
                                    retegas_dettaglio_ordini.prz_dett = (SELECT prezzo FROM retegas_articoli WHERE id_articoli='$new_articolo' LIMIT 1),
                                    retegas_dettaglio_ordini.prz_dett_arr = (SELECT prezzo FROM retegas_articoli WHERE id_articoli='$new_articolo' LIMIT 1)
            ...
            log_me($id_ordine,$id_user,"ORD","LIS","Mod. ORD. $id_ordine, adesso vale $vonuovo",0,$log);
        ...
        for ($i = 1; $i <= $riga; $i++){
            $new_articolo = $corrispondenze["new"][$i] ;
            $old_articolo = $corrispondenze["old"][$i] ;
            $log .= "<hr>ART ".$old_articolo." (".$corrispondenze["cod_old"][$i].")---> ".$new_articolo." (".$corrispondenze["cod"][$i].")<br>";
        }

        log_me($id_ordine,$id_user,"ORD","LIS","FAILED Mod. ORD. $id_ordine, adesso vale $vonuovo",0,$log);
    ...
    function ordine_render_visualizza_ordini_aperti_full($ref_table){
      ...
      <thead>
       ...
       $riga=0;
        ...
        if(id_referente_ordine_proprio_gas($c1,_USER_ID_GAS)==0){
            //echo "C9 = $c9<br>";
     ...
     //   }
       ...
       // $sparkline_data="<span class='inlinesparkline'>".$valori_sparkline."</span>";
        ...
        $info = "   <p>Ditta: <a href='".($RG_addr["form_ditta"])."?id_ditte=$c_ditte'>".e4j($c4)."</a><br>
                    Listino: <a href='".($RG_addr["listini_scheda"])."?id_listino=$c_listini'>".e4j($c3)."</a><br>
                    Tipologia: ".e4j($c_tipologia)."</p>";
      ...
      $h .= " <div class=\"rg_widget rg_widget_helper\">
       ...
       $riga=0;
         while ($row = $db->sql_fetchrow($result)){
              ...
              $importo_totale = valore_arrivato_netto_ordine_user($id_ordine,$id_ut);
         ...
         $h.= "</tbody>
               <tfoot>
                <tr class=\"total destra\">
                    ...
                    <th class=\"destra soldi\">$netto_totale<br>$totalone</th>
       ...
       $riga=0;
         while ($row = $db->sql_fetchrow($result)){
              ...
              $h.= "<tr>";
                ...
                $h.= "<td $col_1>$descrizione<br><span class=\"small_link\">($mis x $euro $prezzo)</span></td>";
                $h.= "<td $col_5>$q_ord</td>";
              $h .="</tr>";
  ...
  return $h;
    ...
    function ordine_render_tabella_gas_partecipanti($ref_table,$id_ordine){
      ...
      global $db,$euro;
      $h .= " <div class=\"rg_widget rg_widget_helper\">
                <div style=\"margin-bottom:16px;\"><h3>Lista GAS</h3>
                    <p>Per vedere solo i gas del tuo DES filtra la lista</p></div>
                <table id=\"$ref_table\">
                    <thead>
                        <tr>
                            <th class=\"sinistra\">&nbsp;</th>
                            <th class=\"sinistra filter-select filter-match\" data-placeholder=\"Tutti i DES\">Des</th>
                            <th class=\"sinistra\">Gas</th>
       ...
       $riga=0;
         while ($row = $db->sql_fetchrow($result)){
              ...
              case "0":
                            ...
                            break;
                       case "1":
                            $stato_gas = "INVITATO, MA MANCA REFERENTE";
                  ...
                  $h.= "<tr>";
                    $h.= "<td $col_1>$img</td>";
                    $h.= "<td $col_1>". des_nome_from_id_gas($row["id_gas"])."</td>";
  ...
  return $h;
    ...
    function ordine_render_partecipa($ref_table,$id,$id_user,$offset=0){
    $euro = "&#0128";
    global $RG_addr,$db;
         ...
         $ordine = $id;
         $ord_query = "ORDER BY io_qta_ord DESC,
                                retegas_articoli.descrizione_articoli ASC,
         ...
         if(!empty($solo_un_articolo)){
            ...
            retegas_listini.id_listini,
             ...
             (SELECT SUM(qta_ord)  FROM retegas_dettaglio_ordini
                        WHERE retegas_dettaglio_ordini.id_utenti = '"._USER_ID."'
                        AND retegas_dettaglio_ordini.id_articoli=retegas_articoli.id_articoli
                        AND retegas_dettaglio_ordini.id_ordine = '$id' )
             AS io_qta_ord
        ...
        //SE E' UN LISTINO CON I TAGS
        if(listino_opz_usage($valore)==1){
            $has_tags = true;
        }else{
            $has_tags = false;
        }
        ...
        if($has_tags){
            $extra_columns_headers ="<th data-placeholder=\"Filtra\" class=\"\">Categoria</th>";
        }
       ...
       $riga=0;       // Contatore per pari e dispari e per id_riga
         ...
         $id_box++;
              ...
              $qta_mia_attuale = $row["io_qta_ord"];
           ...
           if(empty($qta_mia_attuale)){
               ...
               if($has_tags){
                   ...
                   if($row["articoli_opz_1"]<>""){
                                      $op1 = $row["articoli_opz_1"];
                   }else{
                                      $op1 = "";
                   }
                   if($row["articoli_opz_2"]<>""){
                                      $op2 = "<br>".$row["articoli_opz_2"];
                   }else{
                                      $op2 = "";
                   }
                   if($row["articoli_opz_3"]<>""){
                                      $op3 = "<br>".$row["articoli_opz_3"];
                   }else{
                                      $op3 = "";
                   }


                   $extra_columns = "<td><span class=\"small_link\">$op1 $op2 $op3</span></td>";


                   $extra_columns_footer ="<td>&nbsp;</td>";
               }
            ...
            //Mostra le opzioni solo se non sono tags
            if(!$has_tags){
