...
//----------------------------------------CURL
//echo $upfile."<br>"; 
	 # set your csv spreadsheet ur
//$upfile = substr($upfile,strpos($upfile,"key=")+4);

//echo $upfile."<br>";

//$upfile = "https://spreadsheets.google.com/pub?key=".substr($upfile,0,strpos($upfile,"&hl=it"))."&output=csv";
...
if(http_file_exists($upfile)){
    $fd = fopen ($upfile,"r");
    sleep(1);
}else{
 go("sommario",_USER_ID,"Link Errato; Se hai appena pubblicato il documento, potrebbe essere che Google non abiliti subito la lettura.");
...
$fd = fopen ($upfile,"r"); 
		...
		Importazione da GOOGLEDOCS articoli listino
		</div><br>";
		if(($erro_vuoti+$erro_doppi+$error_msg+$erro_num)>0){
			$h_table .="Collegamento a ($upfile) avvenuto correttamente, <b>MA...</b>";
		}else{
			$h_table .="Collegamento a  ($upfile) avvenuto correttamente !....  ";
		}
		...
		if ($erro_num + $erro_vuoti + $erro_doppi + $erro_zero ==0){
			...
			$h_table .="Sembra tutto OK;				
							 <a class =\"large awesome green\" href=\"listini_form.php?do=upload&fname=".urlencode($upfile)."&listino=$listino&tipo_file=GOO&quanti_caricarne=$row\">Carica questi $row Articoli nel listino $listino</a>
		 <br />"; 
		}else{
			
		$h_table .="<div class=\"ui-state-error ui-corner-all padding_6px\"><b>Correggere il vostro listino su Google, ripubblicare sul web, e rifare l'upload.
		</div>
		</br> ";     
		}
		$row = 0;
		...
		<td width=\"5%\">Qta' scatola</td>
		<td width=\"5%\">Qta' minima</td>
		<td width=\"5%\">Note</td>
...
// initialize a loop to go through each line of the file
 while (($data = fgetcsv($fd, 1000, ",")) !== FALSE) {
	...
	$h_table .="<td width=\"2%\">$row</td>";       // numero riga
        ...
        if (ereg("[^A-Za-z0-9.,-_!$%()= ]",  $data[10])>0){$bg[10] = "class=\"punteggiatura\"";}// Descriz COntiene caratteri insulsi  
        if (ereg("[^A-Za-z0-9.,-_!$%()= ]",  $data[11])>0){$bg[11] = "class=\"punteggiatura\"";}// U mis caratteri insulsi  
        if (ereg("[^A-Za-z0-9.,-_!$%()= ]",  $data[12])>0){$bg[12] = "class=\"punteggiatura\"";}// Ingombro caratteri insulsi 
		...
		if (!is_numeric(trim(str_replace(array(",","?","'"),array(".","",""),$data[2])))){$bg[2] = "bgcolor=\"#FF4477\"";}   
			...
			$h_table .="<td><div $bg[2]>$data[2]<div></td>
                 ...
                 <td width=\"5%\"><div $bg[10]>$data[10]<div></td>
                 <td width=\"5%\"><div $bg[11]>$data[11]<div></td>
                 <td width=\"5%\"><div $bg[12]>$data[12]<div></td>";
			$h_table .="</tr>";
