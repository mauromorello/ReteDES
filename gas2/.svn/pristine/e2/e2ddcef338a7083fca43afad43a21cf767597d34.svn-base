...
//$r->menu_orizzontale = ordini_menu_completo($user,$id_ordine);
    ...
    $r->menu_orizzontale[] = '  <li><a class="medium silver awesome">Esporta</a>
                                    <ul>
                                        <li><a class="awesome medium silver"  href="'.$RG_addr["ordini_report_riep_all_gas"].'?id_ordine='.$id_ordine.'&output=html">Versione stampabile</a></li>
                                        <li><a class="awesome medium silver"  href="'.$RG_addr["ordini_report_riep_all_gas"].'?id_ordine='.$id_ordine.'&output=pdf&cod='.rand(0,999999999).'">Pdf</a></li>
                                    </ul>
                                </li>';
    //}
...
while ($row_gas = $db->sql_fetchrow($result)){
    ...
    if($stato==2){
        ...
        $h .="<h3>".$row_gas["descrizione_gas"]."</h3>";
        $h .= "<table>";
        $h .= "<thead>";
            $h .="<tr>";
...
die();
    ...
    $r->contenuto = schedina_ordine($id_ordine)
                    ...
                    <h2>Tutti i GAS (Riepilogo articoli)</h2>"
                    .$h
                    ."</div>";
