...
function do_mod1($id_user,$id_ord,$box_q_ord,$box_q_arr,$box_det_id){
...
ORDER BY
		...
		$titolo_tabella="Conferma articoli ordinati - DETTAGLIO";
		
		
		$output_html .= "   <div class=\"rg_widget rg_widget_helper\">
							<h3>$titolo_tabella<h3>
							<form method=\"POST\" action=\"oc_modifica_q_arr_user.php\">
							<table>        
		...
		$somma_amico = 0;
		 ...
		 while ($row = mysql_fetch_array($result)){
			  ...
			  $tag_articoli ="";
			  $tag_field="<input type=\"text\" name=box_q_arr[] value=$c4 size=\"4\">
...
if (is_logged_in($user)){
    ...
    //SE SONO IL PROPRIETARIO
	 ...
	 // --- menu aperto
	 if($do=="do_mod"){
		 $msg .= do_mod1($id_user,$id,$box_q_ord,$box_q_arr,$box_det_id);
