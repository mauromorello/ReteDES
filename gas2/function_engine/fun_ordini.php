...
if($db->sql_numrows($result_msg)>0){
    ...
    while ($row = $db->sql_fetchrow($result_msg)){
        ...
        while ($row_p = $db->sql_fetchrow($result_prenotazione)){
        ...
        if($row["mail_level"]>0){
            ...
            $message = "L'ordine $ordine ($descrizione) gestito da $gas_name, si è appena chiuso automaticamente.<br>
                         ...
                         ------------------------------------------------------ <br> 
                         Ti ricordo che puoi effettuare delle modifiche all'ordine chiuso, cioè:<br>
                         <br>
                         1. Cambiare le spese di spedizione e di gestione<br>
                         2. Abilitare le pagine delle stampe a tutti gli utenti che hanno partecipato<br>
                         3. Confermare o correggere i quantitativi di articoli arrivati.<br>
                         4. Riaprire l'ordine posticipandone la scadenza.<br>
                         <br> 
                         Buona Distribuzione !!!
                         <br>
                         <br> 
...
function controlla_integrita_ordine_totale($id_ordine){
    ...
    if($row["note_ordini"]<>""){
        $h2 .= rg_toggable("Note Ordine","note_ordine",$row["note_ordini"],false);
    }
    ...
    if(check_option_aiuto_ordine($id_ordine,_USER_ID)==0){
        $offerta_aiuto="<a class=\"awesome small green destra\" href=\"".$RG_addr["aiutanti_offerta_form"]."?id_ordine=$id_ordine\">Offri il tuo aiuto !!</a>";
    ...
    if($valore_globale_attuale_netto_qarr>0){
     ...
     $toggle_note ="<a class=\"small awesome silver destra\"  onclick=\"
         ...
         $h_table .=" <div class=\"rg_widget rg_widget_helper\">
                    ".$toggle_note."<strong>Note ordine</strong>: (clicca sull'icona a destra per visualizzarle o nasconderle)
