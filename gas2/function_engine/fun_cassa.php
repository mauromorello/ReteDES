...
function cassa_render_user_panel($id_utente){
    ...
    $title="Riepilogo Cassa di ".fullname_from_id($id_utente);
    $h .= "<h1>Saldo effettivo: "._nf(cassa_saldo_utente_totale($id_utente)+ abs( cassa_saldo_utente_non_confermato($id_utente)))." Eu.</h1>
           <h3>Saldo non convalidato "._nf(cassa_saldo_utente_totale($id_utente))." Eu.</h3>
           ";
       ...
       $riga=0;
         while ($row = $db->sql_fetchrow($result)){
              ...
              if($visualizza_checkbox==4){
                ...
                $gia_reg =  "<a class=\"awesome small green\" href=\"".$RG_addr["cassa_movimenti_ord_ut"]."?id_ordine=$id_ordine&id_utente=".mimmo_encode($id_ut)."\">Vis</a>";
              }
...
LLL;
   ...
   //ANCHE DI ALTRI GAS
          $q_ord = "SELECT
                    Sum(retegas_dettaglio_ordini.qta_arr * retegas_dettaglio_ordini.prz_dett_arr) as importo_utente,
                    Sum(retegas_dettaglio_ordini.qta_arr) as somma_articoli,
                    Count(retegas_dettaglio_ordini.id_articoli) as conto_articoli,
                    maaking_users.fullname,
                    maaking_users.userid,
...
function cassa_show_totale_suggerimenti_carico(){
  global $db;
  $sql = "SELECT * FROM retegas_options WHERE chiave='PREN_MOV_CASSA' AND id_user='"._USER_ID."'";
  $result = $db->sql_query($sql);
  while ($row = $db->sql_fetchrow($result)){
        $h.="<div>".$row["valore_text"]." - ".$row["note_1"]."; &euro; "._nf($row["valore_real"])." - <a href=\"?do=del&id=".$row["id_option"]."\" class=\"awesome red option\">E</a></div>";
        $val = $val + $row["valore_real"];
  }
  if($val>0){
    $val=_nf($val);
    return "<br>Hai una carica di <b>$val</b> Eu. in attesa del cassiere.";
  }else{
    return;
  }
}