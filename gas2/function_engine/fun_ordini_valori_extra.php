...
$ret = mysql_query($sql);
  ...
  $class_debug->debug_msg[]= "FUN : valore_totale_ordine_qarr id_ordine=$ido result = ".$row[0]; 
  return $row[0];
...
function valore_assoluto_costo_mio_gas($ordine,$gas){
	//echo $ordine." - ".$gas;
	global $db;
	$query = "SELECT * FROM retegas_referenze WHERE id_ordine_referenze='$ordine' AND id_gas_referenze='$gas';";
	$result = $db->sql_query($query);
	...
	return $row["maggiorazione_referenza"];         
}
...
//Valore ARR AMICO :                    Parametri   : Ordine, User, Amico
...
//Valore ARR AMICO COSTO TRASPORTO :    Parametri   : Ordine, User, Amico
//Valore ARR AMICO COSTO GESTIONE :     Parametri   : Ordine, User, Amico
...
function valore_totale_mio_ordine($id_ordine,$id_user){
...
$ret = $db->sql_query($sql);
  $row = mysql_fetch_row($ret);
  $class_debug->debug_msg[]="FUN : valore_totale_mio_ordine id_ordine = $id_ordine, id_user = $id_user, Result : ".$row[0];
  return $row[0];
...
function valore_totale_ordine_new($id_ordine){
global $db;
Global $class_debug;    
...
$ret = mysql_query($sql);
  ...
  $class_debug->debug_msg[]= "FUN : valore_totale_ordine_new id_ordine=$id_ordine result = ".$row[0]; 
  return $row[0];
