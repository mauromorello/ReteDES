...
// Se $new_name ט vuota, il nome sarא lo stesso del file uploadato
...
//$h_table .= "L'upload del file " . $_FILES["upfile"]["name"] . " ט avvenuto correttamente";
$row = 0;
...
// ------------------------------------------------------------------------------------------------------------------------
		...
		Importazione da file MS EXCEL articoli listino
		</div><br>";
		...
		if ($erro_zero>0){ 
		$h_table .="<div class=\"qta_inf_1 ui-corner-all\">n.$erro_zero Quantita' inferiori a 1 (il minimo ט 1))</div>"; 
		}
		if ($erro_multi>0){ 
		...
		if ($erro_num + $erro_vuoti + $erro_doppi + $erro_zero + $erro_opzioni ==0){
			// UPLOAD :
			// <a class =\"large awesome green\" href=\"listini_form.php?do=upload&fname=$filone&listino=$listino&tipo_file=XLS\">Carica questi $row Articoli nel listino $listino</a> 
			
			$h_table .="Sembra tutto OK; <a class =\"large awesome green\" href=\"listini_form.php?do=upload&fname=$filone&listino=$listino&tipo_file=XLS\">Carica questi $row Articoli nel listino $listino</a>		
...
for ($i_x = 2; $i_x <= $data_xls->sheets[0]['numRows']; $i_x++) {
	...
	$h_table .="<td width=\"2%\">$row</td>";       // numero riga
		...
		if (!is_multiplo((trim(str_replace(array(","),array("."),$data_xls->sheets[0]['cells'][$i_x][8]))),(trim(str_replace(array(","),array("."),$data_xls->sheets[0]['cells'][$i_x][7]))))){$bg[8] = "class=\"multiplo_errato\"";} // Non ט un multiplo 
