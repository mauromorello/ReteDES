...
if (!_USER_LOGGED_IN){
    ...
    if($do=="reg"){
          $ok=0; $err_db =0;
          while (list ($key,$val) = @each ($box_register)) {
                //Echo "box_id_op = KEY $key Id OP $val<br>";
          $sql = 'UPDATE `retegas_cassa_utenti` SET `id_cassiere` = \''._USER_ID.'\', `registrato` = \'si\', `data_registrato` = NOW() WHERE `retegas_cassa_utenti`.`id_cassa_utenti` = '.$val.' LIMIT 1;';      
          ...
          $msg = "Registrati n. <strong>$ok</strong> movimenti;<br>
    ...
    $retegas->posizione = "Movimenti da Registrare";
    ...
    $retegas->java_headers = array("rg");  
      ...
      $retegas->content  =  cassa_movimenti_registrare(_USER_ID_GAS,$id_ordine);
