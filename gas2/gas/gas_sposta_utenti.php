    ...
    $retegas->java_headers = array("rg");  // editor di testo
      ...
      if(isset($msg)){
       ...
       $h ='<div class="rg_widget rg_widget_helper">
        ...
        <select name= "id_utente_target" id="selection">';
        if(_USER_PERMISSIONS & perm::puo_vedere_retegas){
            $result = $db->sql_query("SELECT * FROM maaking_users inner join retegas_gas on retegas_gas.id_gas = maaking_users.id_gas WHERE id_des='"._USER_ID_DES."' AND userid<>'"._USER_ID."' ORDER BY fullname ASC");
            $msg_p = "<p>Hai i permessi per gestire il des, puoi spostare utenti di altri gas.</p>";
        }else{
            $result = $db->sql_query("SELECT * FROM maaking_users WHERE id_gas='"._USER_ID_GAS."' AND userid<>'"._USER_ID."' ORDER BY fullname ASC");
            $msg_p = "<p>Puoi solo spostare utenti del tuo GAS.</p>";
        }
